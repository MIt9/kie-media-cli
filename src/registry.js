/**
 * Dynamic model registry: live catalog from https://docs.kie.ai/llms.txt
 * (market pages section), cached at ~/.kie-media/models-cache.json (TTL 24h),
 * merged with seed registry (src/models.js).
 *
 * Fallback chain: fresh cache → stale cache → built-in seed.
 * Seed takes precedence for metadata when IDs match; dynamic discovery adds
 * new models and confirms existence of seed models.
 */
...
export const CACHE_PATH = path.join(os.homedir(), ".kie-media", "models-cache.json");
...
/**
 * Parses llms.txt → list of market pages:
 * [{ url, category, title, description }]. /cn/ links and Chat Models are skipped.
 */
...
/**
 * Extracts model ID from documentation markdown page.
 * Returns array of IDs sorted by frequency (first is primary).
 */
...
/**
 * Merges live catalog with seed registry.
 * liveEntries: [{ id, category, description, docUrl }].
 * Returns Map<id, entry>.
 */
...
/** Fetches documentation markdown page (docs.kie.ai/market/...). */
...
/**
 * Fetches live catalog: llms.txt + all market pages (concurrency 8).
 * Returns [{ id, category, description, docUrl }].
 */
...
/**
 * Loads registry with caching:
 * { models: Map<id, entry>, source: live|cache|seed, fetchedAt }.
 */
export async function loadRegistry({
  ...
  if (shouldFetch) {
    try {
      ...
      if (onWarning) onWarning(`Failed to refresh model catalog (${exc.message}).`);
    }
  }
